...
### Algorithms
  ...
  * **Hill Climbing** → A simple linear policy optimized by adding noise and adopting changes that lead to higher rewards.
  * **DDQN** → A PyTorch-based neural network with two hidden layers (64 neurons each, ReLU activation).
  * **Hybrid DDQN** → The DDQN agent's replay buffer is pre-filled with experiences generated by the trained Hill Climbing agent.
...
### Training Setup
  ...
  * **Exploration Strategy**: Epsilon-greedy, with epsilon decaying from `1.0` to `0.01`.
  * **Hybrid Pre-fill**: Approximately 50 episodes from the Hill Climbing agent are used to populate the initial replay memory.

-----

## 🔮 Future Work

  * [ ] Add PPO / A2C algorithms for a more comprehensive comparison.
  * [ ] Implement hyperparameter tuning using libraries like Optuna or Ray Tune.
  * [ ] Extend the hybrid pre-fill concept to more complex environments (`LunarLander-v2`, `MountainCar-v0`).
  * [ ] Add functionality to save and load trained model weights.
  * [ ] Develop a web-based user interface for demonstrations using Streamlit or Gradio.
